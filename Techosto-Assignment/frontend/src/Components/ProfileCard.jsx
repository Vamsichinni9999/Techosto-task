...
const ProfileCard = ({ user, setUsers }) => {
  ...
  const onFinish = (values) => {
    axios
      .put(
        `https://techosto-assignment-task.onrender.com/api/edit-user-details/${user._id}`,
        values
      )
      .then((response) => {
  ...
  const handleDelete = () => {
    axios
      .delete(
        `https://techosto-assignment-task.onrender.com/api/delete-user/${user._id}`
      )
      .then((response) => {
        if (response.status === 200) {
    ...
    <div style={{ margin: "10px" }}>
      <Card
        ...
        actions={[
          ...
          <Button
            type="text"
            icon={<DeleteFilled style={commonIconStyle} />}
